...
int[][][] animLine(){
  ...
  for (int i = 0; i < nPairs; i++) {
    ...
    targets[i][0][0] = (int) (((xmax + xmin)/ 2) + (cos(i * PI) * lineOffset));
    targets[i][0][1] = ystart;
    targets[i][0][2] = 0;
    
    
    targets[i][1][0] = (int) (((xmax + xmin)/ 2) + (cos(i * PI) * -lineOffset));
...
int[][][] animRotateLine(){
  ...
  for (int i = 0; i < nPairs; i++) {
    ...
    targets[i][0][0] = (int) ((xmax + xmin)/2 + (r * sin(globalAngleOffset)));
    targets[i][0][1] = (int) ((ymax + ymin)/2 + (r * cos(globalAngleOffset)));
    targets[i][0][2] = 0;
    
    
    targets[i][1][0] = (int) ((xmax + xmin)/2 + (r * sin(globalAngleOffset)));
    targets[i][1][1] = (int) ((ymax + ymin)/2 + (r * cos(globalAngleOffset)));
    targets[i][1][2] = 0;
  }
  println();
